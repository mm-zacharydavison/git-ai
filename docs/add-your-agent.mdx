...
Now you just need to get your agent to call `git-ai checkpoint` the same way to mark its edits. Before building your own checkpoint preset, you can wire everything up with the `mock_ai` preset for testing. 
...
Right before your agent edits a file use the `agent-v1` preset to mark any other changes as human: 
...
While not required, it's recommended that you pass in the paths of the files your agent will edit. This will speed up the checkpoint process by ~50-100x in large repos by allowing Gi AI to narrow its diff to only files AI has edited since the last commit. 
...
After your agent edits a file you need to call the `agent-v1` preset to mark the edits as AI. You'll need to pass in the transcript of the AI conversation, the agent name, model, and conversation id.

- `transcript` - This is the transcript for the AI thread. We strongly suggest filtering out tool results from the transcript on your side, before invoking `git-ai checkpoint`. Keep sending the full transcript to Git AI, not just new messages. [Struct defined here](https://github.com/acunniffe/git-ai/blob/main/src/authorship/transcript.rs)
...
If you need something more advanced you might want to build your own preset (see the [Advanced - Hooks and Presets](#advanced---hooks-and-presets) section below). 
...
All agents that integrate with Hooks are required to implement their own agent preset: `git-ai checkpoint <your-agent-name>`
